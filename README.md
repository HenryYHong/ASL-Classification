...
└── docs/                        the hosted demo, served by GitHub Pages. See docs/README.md.
    ...
    ├── models.bin.gz            the same three forests as packed binary — what the page fetches, 1.18 MB on the wire
    ├── models.json              the same forests as readable JSON, 20.4 MB; the reference copy, no longer fetched
...
| --- | --- | --- |
| **Static letters, leave-one-SIGNER-out** | **0.9406** ± 0.0033 over 3 seeds; at seed 0, 0.945 over all 23,984 held-out frames, per signer 0.880 to 1.000 | `temporal/crossval_signers.py`: hold out all 2,400 frames of one of ASL-HG's ten signers, train on the other nine plus my sessions and the other stranger sets. The first by-signer number the letters have had |
...
| **Static letters, the permanent never-train holdout** | **0.889** on the shipped forest (signer-clustered 95% CI [0.853, 0.915]); the previous release's forest reads **0.797** on the same 1,111 frames, and the paired difference is **+0.0918, CI [+0.069, +0.134]**, all five signers improving | the CC0 `ayuraj` set, five signers. Nothing in this repository may ever train on it, so the two figures are the same measurement. **All of that difference is the new dataset, none of it the recipe** — see below. `crossval_strangers.py --compare-prev <old pickle>` prints the pair; the interval resamples the five SIGNERS, not the 1,111 frames, which would claim ±0.019 it has not earned |
| **A stranger holding a letter through the whole runtime** | 266 video clips: exactly right on 96, silent on 170, **0 wrong letters**. On the 139 clips tracked at least 60% of frames: 59 right, 0 wrong. On the 58 of those lasting at least a second: 33 right, 0 wrong | `temporal/replay_strangers.py` over the OpenHands fingerspelling clips, one fresh segmenter and one fresh MediaPipe per clip, shipped models and thresholds. The clips carry no signer ids, so this is not a by-signer number |
...
| Segmenter on my 71 held-out holds | exactly the right letter 68 of 71, one wrong letter in 71 holds, two silent; 22 of 24 on the cross-day fold — identical at all three seeds | `temporal/replay_static.py`: fold models, real timestamps, one fresh segmenter per hold |
| A relaxed hand, 43 idle holds from a live browser log | 0 emissions and 0 of 2,064 votes **for the forest that ships** (seed 0). The RECIPE is not that clean: over eight seeds it emits at four of them | `temporal/idle_gate.py --seeds 8`: one signer, one stretch of about two minutes. the eight-seed table is below, under the vote floor |
...
What that buys, replaying my 71 held signs end to end through the segmenter with real timestamps and forests that never saw the session, at every one of three seeds: exactly the right letter on **68 of 71** against 61 for the flat 0.75 it replaces, **2 holds silent** against 9, cross-day 22 of 24 against 20, one wrong letter either way, and 0 of the 43 idle holds either way. The two that stay silent are S2-K, which is silent under every forest and every floor this project ships, and S1-G — G's own hold, paying G's own bill, which is the honest shape of the trade. A flat 0.55 would give 69 of 71 and emit on 4, 4 and 5 of the 43 idle holds (53 to 55 votes, every one a G).
...
**And three was the same mistake one level up.** I picked seeds 0, 1 and 2, they were clean, and I called that a gate. Run at eight, the shipped recipe emits on a clean idle hold at seeds **3, 4, 5 and 7** — four of the eight — and the idle G maximum runs 0.6816 to 0.7940 against its 0.75 floor. Which three seeds you happen to pick decides whether the gate prints PASS. The forest that actually ships is seed 0 and genuinely reads 0 of 43, so the page is not emitting letters at a resting hand; what was wrong was the claim that the recipe is robust, not the artifact. Per-letter idle maxima over those eight seeds, against the floor each letter is given:

| letter | seeds seen | worst idle | its floor | headroom |
| --- | --- | --- | --- | --- |
| G | 8 | 0.7940 | 0.75 | **-0.0440** |
| Q | 3 | 0.5504 | 0.55 | **-0.0004** |
| A | 8 | 0.5310 | 0.55 | +0.0190 |
| O | 8 | 0.5105 | 0.55 | +0.0395 |
| M | 7 | 0.4524 | 0.55 | +0.0976 |

Two floors are breached and a third has 0.019 of room. Floors that hold over all eight draws are flat **0.60** with G at **0.85**, and that is not free: replayed on the 71 holds it gives 65 exact and 5 silent against 68 and 2, the extra three being S3-D and G's own two S3 holds. `idle_gate.py` now refuses to print PASS under `MIN_SEEDS = 8` and says so, because a clean three-seed run is exactly what this recipe looked like before it was asked a fourth question.
...
The forests ship in a packed binary form this release, `docs/models.bin.gz` plus a small `models.meta.json`, and that is what the page fetches. Same numbers, 3.1x smaller on the wire than the JSON (1,184,403 B against 3,703,057 B) and four to six times faster to decode (18-38 ms against 114-160 ms in Node), with zero routing differences over all 278,750 nodes and a worst probability difference of 4.98e-7 against sklearn on the 41 golden frames, against 3.33e-6 through the JSON. `docs/models.json` is still exported and still committed, because a text file is what makes the trees readable and what every other tool here can open, and `loadModels` returns the identical object from either. `docs/README.md` has the format and the measurements.
...
./.venv/bin/python temporal/idle_gate.py --seeds 3                  # a HARD GATE: must pass at every seed
