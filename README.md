...
The third approach is hosted at **[henryyhong.com/ASL-Classification](https://henryyhong.com/ASL-Classification/)**. There is nothing to install, and the camera frames stay on your machine: the page downloads the models — `models.bin.gz` and a small `models.meta.json`, **1,184,420 B on the wire**, about 1.18 MB — and runs them locally. Chrome and Safari both work. Allow the camera, then sign into the box. The forest grew 22.8% this release and the download shrank anyway, because the three forests now ship packed instead of as JSON. JSON is the form the page fetched until this release, and the same trees in it are 20.4 MB raw and 3,703,069 B gzipped, which is how GitHub Pages would serve them. `docs/models.json` stays committed as the readable reference; `docs/README.md` has the format and the parity measurements.
...
| **Static letters, the permanent never-train holdout** | **0.889** on the shipped forest; the previous release's forest reads **0.797** on the same 1,111 frames | the CC0 `ayuraj` set, five signers. Nothing in this repository may ever train on it, so the two figures are the same measurement and the difference between them is the retrain |
...
| A relaxed hand, 43 idle holds from a live browser log | 0 emissions and 0 of 2,064 votes, at all three seeds | `temporal/idle_gate.py`: one signer, one stretch of about two minutes |
...
The forests ship in a packed binary form this release, `docs/models.bin.gz` plus a small `models.meta.json`, and that is what the page fetches. Same numbers, 3.1x smaller on the wire than the JSON (1,184,420 B against 3,703,069 B) and four to six times faster to decode (18-38 ms against 114-160 ms in Node), with zero routing differences over all 278,750 nodes and a worst probability difference of 4.98e-7 against sklearn on the 41 golden frames, against 3.33e-6 through the JSON. `docs/models.json` is still exported and still committed, because a text file is what makes the trees readable and what every other tool here can open, and `loadModels` returns the identical object from either. `docs/README.md` has the format and the measurements.
